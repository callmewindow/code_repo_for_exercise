...
// 使用i记录是否第一次访问，快了一点
function dfs(grid: number[][], vis: number[][], row: number, col: number, flag: number, visFlag:number): number{
  // console.log(row, col);
  let res = 0;
  // 如果已经访问或者超出边界则返回0
  if(row < 0 || row >= grid.length || col < 0 || col >= grid[0].length) return res;
  ...
  // 寻找其他数
  res += dfs(grid, vis, row+1, col, flag, visFlag);
  res += dfs(grid, vis, row, col+1, flag, visFlag);
  res += dfs(grid, vis, row-1, col, flag, visFlag);
  res += dfs(grid, vis, row, col-1, flag, visFlag);
...
function maxPoints(grid: number[][], queries: number[]): number[] {
  ...
  let answer = Array(q).fill(0);
  for(let i = 0;i<q;i++){
    answer[i] = dfs(grid, vis, 0, 0, queries[i], i); // 用i来标记是这次的第一次访问
  }
  return answer;
};