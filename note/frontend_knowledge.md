...
例如，当使用v-model指令绑定一个表单输入框时，Vue会为该表单输入框的value属性定义getter和setter方法。当输入框的内容发生变化时，Vue会触发setter方法，从而更新对应的数据，并重新渲染视图。
...
如果需要动态指定include或exclude，可以使用:include和:exclude的绑定语法，将一个数组或者一个返回数组的函数绑定到属性上。
...
模板语法：Vue使用模板语法，即在模板中使用类似HTML的语法来创建视图。而React使用JSX语法，即将HTML和JavaScript混合在一起来创建视图。
...
Flex的优势在于它能够快速地实现一维的布局，即在水平或垂直方向上排列元素。通过设置flex容器和子元素的属性，比如flex-direction、justify-content和align-items等，可以很容易地控制元素的位置和对齐方式。因此，Flex常用于构建导航栏、列表、卡片等基于一维布局的组件。此外，Flex还可以通过flex-wrap属性实现元素的换行，同时通过flex-grow、flex-shrink和flex-basis属性实现元素的伸缩性。

Grid的优势在于它能够实现二维布局，即在行和列方向上排列元素。通过设置grid容器和子元素的属性，比如grid-template-rows、grid-template-columns和grid-template-areas等，可以很容易地控制元素在网格中的位置和对齐方式。因此，Grid常用于构建页面布局，比如网格式布局、响应式布局等。Grid还可以通过grid-gap属性实现元素之间的间距，同时通过grid-auto-rows、grid-auto-columns和grid-auto-flow属性实现自动布局和元素的伸缩性。
...
变量可以定义在任何选择器内，使用var()函数进行调用。定义变量时，以--开头，后面跟着变量名和变量值

var() 函数的第一个参数是自定义属性的名称，例如：
...
在这个例子中，var(--main-color) 引用了自定义属性 --main-color，var() 函数会按照下面的顺序查找对应的值：

查找当前元素的内联样式中是否有对应的自定义属性值；
如果没有，查找当前元素的父元素的内联样式中是否有对应的自定义属性值，一直向上查找直到找到为止；
如果还是没有找到，查找全局作用域中定义的自定义属性值（在 :root 伪类中定义的自定义属性值属于全局作用域）。

如果最终还是没有找到对应的自定义属性值，那么 var() 函数会使用第二个参数作为默认值。如果没有提供默认值，则使用 initial 作为默认值。
...
通过伪元素::before和::after，我们可以在不改变HTML文档的前提下为选中元素添加内容。它们的主要作用是通过CSS来修饰选中元素的内容，以及提供更多的样式上的灵活性。可以使用content属性来定义插入的内容。
...
当 JavaScript 需要将一个对象转换为原始值时，会调用该对象的 valueOf 和 toString 方法，以确定对象应该转换为哪种类型的原始值。

Object.prototype.toString 方法是一个特殊的方法，用于检测对象的类型。该方法会返回一个由 "[object" 和类名和 "]" 组成的字符串。其中，类名即为对象的原型链上最后一个构造函数的名称，或者是 Object（如果对象的原型链上没有其他构造函数）。

当使用 toString 方法将一个对象转化为字符串时，JavaScript 引擎会先调用对象的 valueOf 方法，如果该方法返回的是原始值，则直接返回该原始值的字符串形式。如果 valueOf 方法返回的是对象类型的值，则再调用对象的 toString 方法来获取字符串。

由于大部分 JavaScript 内置对象都覆盖了 toString 方法，因此 Object.prototype.toString 可以成功地返回对象的类型。当然，如果想要自定义一个对象的类型标识字符串，也可以通过重写 toString 方法来实现。

至于 toPrimitive 方法，它是 ECMAScript 6 中新增的一个方法，用于将一个值转换为原始类型的值。如果一个对象没有实现 valueOf 方法，或者该方法返回的不是原始类型的值，则会调用该对象的 toString 方法。toPrimitive 方法接受一个参数，可以指定转换的目标类型，可以是 "number"、"string" 或 "default"（默认值）。如果不传入参数，则默认为 "default"。

综上所述，JavaScript 中的对象输出为 "[object Object]" 是因为其 toString 方法返回了 "[object Object]" 的字符串形式。如果想要改变对象输出的字符串形式，可以通过重写对象的 toString 方法来实现。
...
和基于 json 的转化
基于 json 的深拷贝有风险：会忽略 undefined、symbol 和函数
...
setTimeout() 函数被调用，并将指定的代码块及延迟时间添加到 JavaScript 引擎的任务队列中。（会在指定时间后将任务加入到异步任务队列中）
JavaScript 引擎会继续执行当前任务，直到所有任务完成。
当 JavaScript 引擎没有任务执行时，会检查任务队列中是否有代码块需要执行，如果有，则按照添加顺序依次执行。（即会在所有的同步任务执行完毕后才会执行）
当 JavaScript 引擎执行到任务队列中 setTimeout() 对应的代码块时，会将其从队列中移除，并执行相应的代码。

虽然使用 setTimeout() 可以指定延迟时间，但是并不能保证代码一定会在指定的时间内执行，因为 JavaScript 引擎执行任务的优先级是不确定的，而且还受到其他因素的影响，如系统负载、网络延迟等。因此，使用 setTimeout() 时需要注意一些潜在的问题，如延迟时间过长可能会导致页面卡顿等。
...
Set
使用 Set 对象可以快速去除数组中的重复项，Set 对象会自动过滤重复的值。可以将数组转换成 Set 对象，然后再将 Set 对象转回数组。这种方式的优点是代码简洁，缺点是无法保证原数组中元素的顺序。

indexOf
使用 indexOf 方法遍历数组，如果在当前索引之前已经出现过该元素，则删除该元素。该方式的优点是保证了原数组中元素的顺序，缺点是效率低下，不适合处理大型数组。

includes
使用 includes 方法遍历数组，如果在当前索引之前已经出现过该元素，则删除该元素。该方式的优点是代码简洁，缺点是效率低下，不适合处理大型数组。

Map
使用 Map 对象也可以去重，可以将数组元素作为 Map 对象的键值，然后取出 Map 对象中的键值即可。该方式的优点是保证了原数组中元素的顺序，缺点是代码较为复杂。

reduce
使用 reduce 方法遍历数组，如果在当前索引之前已经出现过该元素，则删除该元素。该方式的优点是代码简洁，缺点是效率较低，不适合处理大型数组。
...
原型链继承
比较常见的继承方式之一，其中涉及的构造函数、原型和实例，三者之间存在着一定的关系，即每一个构造函数都有一个原型对象，原型对象又包含一个指向构造函数的指针，而实例则包含一个原型对象的指针
...
引用属性不会被共享，优化了第一种继承方式的弊端，但是只能继承父类的实例属性和方法，不能继承原型属性或者方法
...
原型链是 JavaScript 中的一个重要概念，用于实现继承和属性查找。每个 JavaScript 对象都有一个原型，对象可以从其原型继承属性和方法。
如果在对象自身上没有找到所需的属性或方法，它会在其原型上继续查找，直到找到为止。这样一直延续下去就形成了原型链。
...
在 JavaScript 中，每个对象都有一个内部链接到其原型对象的属性 \__proto__。
每个对象的__proto__都是指向它的构造函数的原型对象prototype的
原型对象本身也是一个对象，同样拥有自己的 \__proto__，这样就可以形成一条链，称为原型链。
原型链最终指向的是 Object.prototype，它是所有对象的最终原型，包含了 JavaScript 内置的一些方法和属性，如 toString 和 valueOf 等。

Property 是一个表示对象属性的抽象概念，可以是数据属性或访问器属性。
数据属性是包含一个数据值的位置，在对象上直接存储和访问。
访问器属性是指一对 getter 和 setter 函数，可以在读写属性时触发自定义的操作。
对象的属性既可以在对象本身上定义，也可以通过原型链继承自其原型对象。
在查找属性时，JavaScript 引擎会先查找对象本身是否包含该属性，如果没有再到其原型链上查找。

区别在于 \__proto__ 是一个属性，指向对象的原型对象，是对象内部实现原型继承的机制；
而 Property 是一个抽象概念，表示对象的属性，包括数据属性和访问器属性。
在 JavaScript 中，对象的属性既可以在对象本身上定义，也可以通过原型链继承自其原型对象。
...
一个类数组对象，包含了函数调用时传入的所有参数。它只在函数内部可用，并且是自动创建的，可以通过下标来访问每个参数。

arguments 对象的使用场景是在函数的参数个数不确定时，可以使用它来访问传入的参数。在 ES6 中，我们可以使用剩余参数语法（rest parameters）来代替 arguments

在 ES6 中，我们可以使用解构赋值（destructuring assignment）和剩余参数语法（rest parameters）来更方便地访问函数的参数。
...
1. 代码更加简洁，使用 const 和箭头函数可以让代码更加简洁，没有了 function 关键字和花括号，减少了不必要的代码量，使得代码更加易读易懂。

2. 箭头函数没有自己的 this 和 arguments，会继承外层作用域的 this 和 arguments，使得作用域链更加清晰。

3. 箭头函数不会创建自己的执行上下文，也不会改变 this 的指向，因此对于一些需要在回调函数中使用外层作用域的 this 的场景，使用箭头函数可以避免 this 指向错误的问题。

4. 箭头函数在一些场景下可以更加高效，因为它不需要创建自己的执行上下文和 arguments 对象，使得函数执行更加快速。

总的来说，使用 const 和箭头函数声明函数可以使得代码更加简洁，易读易懂，同时避免了一些常见的作用域和 this 指向问题，可以提高开发效率和代码质量。

#### this的指向

箭头函数中this的指向通常是由定义函数时的作用域决定的，而不是由函数的调用方式决定的。

全局作用域
在全局作用域中使用箭头函数时，this指向的是全局对象。

对象方法
当箭头函数作为对象方法时，this指向的是该对象。

作为普通函数使用
当箭头函数作为普通函数使用时，它的this指向的是它所在的作用域。
...
原始事件模型又称为IE事件模型，主要应用于旧版的IE浏览器；标准事件模型也称为DOM2级事件模型，主要应用于现代浏览器。

在原始事件模型中，事件绑定是通过在DOM元素上使用一个特殊的属性来实现的，例如 element.onclick = function() { ... }。
这种绑定方式的缺点在于，无法为同一个元素的同一事件绑定多个处理函数，因为这个属性只能绑定一个函数。
此外，事件处理程序的执行顺序是不确定的。

在标准事件模型中，事件绑定是通过 addEventListener() 方法来实现的，例如 element.addEventListener('click', function() { ... })。
这种绑定方式可以为同一个元素的同一事件绑定多个处理函数，并且可以通过 removeEventListener() 方法来移除绑定的事件处理程序。
另外，事件处理程序的执行顺序是确定的，遵循先绑定先执行的原则。

这两种事件模型不能同时使用。在现代浏览器中，原始事件模型已经被标准事件模型所取代，因此不建议使用原始事件模型。
...
前面讲到，事件流的都会经过三个阶段： 捕获阶段 -> 目标阶段 -> 冒泡阶段，而事件代理和委托就是在冒泡阶段完成
...
事件代理中，事件处理程序是添加到祖先元素上，通过事件冒泡的机制来处理子元素上的事件。当子元素上的事件被触发时，事件会冒泡到祖先元素，然后再通过判断事件触发的元素来确定要执行哪个事件处理程序。
...
// 这时候就可以事件代理，把点击事件绑定在父级元素ul上面，然后执行事件的时候再去匹配目标元素
// 给父层元素绑定事件
...
使用事件委托
事件委托中，事件处理程序是添加到共同的父元素上，通过事件捕获的机制来处理子元素上的事件。当子元素上的事件被触发时，事件会从祖先元素一直捕获到子元素，然后再通过判断事件触发的元素来确定要执行哪个事件处理程序。
...
事件委托是更广义的概念，可以代替事件代理，而事件代理则是一种实现事件委托的具体方式。
它们的本质区别在于事件委托通常是将事件处理函数委托给另一个对象处理，而事件代理则是利用了事件冒泡机制，将事件处理函数绑定在父元素上，实现对子元素的处理。
...
串行：使用Promise的then方法来处理串行操作。例如，当我们需要按照一定的顺序执行一些异步操作时，可以使用then方法来依次执行。

并行：使用Promise.all方法来处理并行操作。例如，当我们需要同时执行多个异步操作并等待所有操作完成后才进行下一步时，可以使用Promise.all方法。
同时，我们还可以使用Promise.race方法来处理并行操作。例如，当我们只需要等待多个异步操作中的其中一个完成后就进行下一步时，可以使用Promise.race方法。

###### Promise的all的实现

Promise.all() 是一个常用的 Promise 静态方法，它接受一个 Promise 实例数组，返回一个新的 Promise 实例。

Promise.all() 的作用是在所有 Promise 实例都成功时，才会返回成功结果，其中只要有一个 Promise 实例失败，则会直接返回失败结果。也就是说，它实现了 Promise 实例的并行执行。

具体来说，Promise.all() 内部是使用了 Promise 构造函数的一个参数为 resolve 的 Promise 实例来实现的。
当 Promise.all() 接收到一组 Promise 实例时，它会在这些 Promise 实例上调用 then 方法，并将它们的结果存储在一个数组中，当这个数组中的所有 Promise 都成功时，Promise.all() 的返回的 Promise 实例会调用 resolve 方法并将这个数组作为结果传递给它，如果其中有一个 Promise 失败了，返回的 Promise 实例会调用 reject 方法并将这个失败的 Promise 的结果作为参数传递给它。

Promise.all()返回一个promise对象，当所有promise都完成时，这个promise对象才会被resolve，并返回一个结果数组，数组中的结果按照传入Promise.all()的Promise数组中的顺序排列。如果其中有任意一个Promise被reject，则整个Promise.all()也会被reject。
...
await 的实现原理是将其后面的表达式通过 Promise.resolve() 方法转化为一个 Promise 对象，然后将其暂停所在的 async 函数的执行，将控制权交给函数外的代码，等到 Promise 对象的状态发生变化，再将控制权交回来，继续执行后面的代码。如果 Promise 对象被拒绝（rejected），那么将抛出一个错误（使用 throw 关键字），可以通过 try...catch 语句来捕获。

await 可以理解为是 Promise 的语法糖，其实现原理也是基于 Promise 对象的。

具体来说，当遇到 await 关键字时，会将其后面的表达式通过 Promise.resolve() 方法转化为一个 Promise 对象，然后调用该 Promise 对象的 then() 方法，将后续的代码放入到一个回调函数中，然后暂停所在的 async 函数的执行，并将控制权交给函数外的代码。

await 是一个异步操作，它等待一个 Promise 对象完成（或一个实现了 Promise 对象接口的对象），然后返回该 Promise 的结果。在使用 await 的函数中，执行 await 后面的代码时，如果该 Promise 的状态为 pending，则会将后续的代码添加到事件循环队列中，等待该 Promise 对象的状态变更，一旦变成 resolved 或 rejected，则该异步操作就结束了，事件循环机制会将该异步操作的回调函数加入到任务队列中，等待执行。

在事件循环的考察中，await 的效果表现为将异步操作放入事件循环队列中，然后暂停当前函数的执行，等待异步操作的结果返回后再继续执行。这样可以避免浏览器的主线程被阻塞，提高页面的响应速度。
...
可以使用JavaScript动态创建一个script标签，方法如下：
...
其中，your_script_url为需要加载的脚本地址。

要删除一个动态创建的script标签，可以使用removeChild方法，将其从DOM树中移除，代码如下：

```javascript
const script = document.querySelector('script[src="your_script_url"]');
if (script) {
  document.body.removeChild(script);
}
```

其中，your_script_url为需要删除的脚本地址。如果页面上存在多个同样的script标签，需要使用querySelectorAll方法来获取所有匹配的标签，并循环遍历进行删除。

### 获取高宽等信息

offsetWidth/offsetHeight
这两个属性可以获取元素的宽度和高度，包括边框和内边距和滚动条，但不包括外边距。

clientWidth/clientHeight
这两个属性可以获取元素的宽度和高度，包括内边距但不包括边框、外边距和滚动条。

getBoundingClientRect()
这个方法可以获取元素的位置、宽度和高度等信息，相对于视口左上角的位置。
getBoundingClientRect()返回的是一个包含四个属性的矩形对象，即left、top、right和bottom，可以通过这些属性计算出x、y、宽度和高度。

scrollWidth/scrollHeight
这两个属性可以获取元素的内容区域的宽度和高度，包括滚动条，但不包括边框和内边距。
...
// 计算元素与父元素的距离：
const distanceX = parentWidth / 2 - elementWidth / 2;
const distanceY = parentHeight / 2 - elementHeight / 2;
// 设置元素的位置：
...
这样就可以让元素在父元素中居中显示了。需要注意的是，父元素需要设置position: relative;属性，否则元素的位置将以页面为准而不是以父元素为准。

#### 获取浏览器高宽

可以使用window.innerHeight和window.innerWidth属性来获取浏览器可视区域的高和宽，即浏览器窗口大小，不包括浏览器工具栏和滚动条。

window.innerWidth和window.innerHeight获取的是当前窗口的可视区域的宽度和高度，包括滚动条等元素，可以用来处理vw和vh。当浏览器窗口大小改变时，这两个属性也会随之改变。不过需要注意的是，这两个属性并不包括浏览器工具栏、标签栏等非网页内容所占用的区域。
...
cookie
存储大小限制为4KB左右
存储内容会随每次 HTTP 请求发送给服务器，会对网络性能造成影响
可以设置过期时间，到期自动失效
可以设置为HttpOnly，防止XSS攻击

sessionStorage
sessionStorage：存储容量相对较小，会话结束时会自动失效

localStorage
存储容量较大（一般为5-10MB），永久保存在本地，除非手动清除或者浏览器清除缓存

indexedDB
HTML5 标准中的一个 API，它提供了一个非常强大、可靠的浏览器端数据库，可以存储大量数据，并且支持丰富的查询和排序功能。它可以在客户端直接操作数据，不需要与服务器端进行频繁的交互，因此在离线应用、缓存大量数据等场景下非常有用。
...
定义了一个cleanLocalStorage函数，用来遍历localStorage中的所有项，如果发现某项的expires属性小于当前时间，就将其删除。然后我们使用setInterval函数，每隔一天执行一次cleanLocalStorage函数，即可实现localStorage的自动清理。
...
setInterval(cleanLocalStorage, 24 * 60 * 60 * 1000); // 每隔一天清理一次
```

使用浏览器自带的过期时间机制。localStorage和sessionStorage都支持设置一个过期时间，超过这个时间后，存储的数据会自动失效。这样可以避免定时器的误差和对性能的影响。

具体实现方式可以在存储数据的时候同时存储一个过期时间，然后在读取数据的时候判断一下是否过期，如果过期了就删除数据。

```js
// 存储数据时，同时存储过期时间
function setItem(key, value, expired) {
  let item = { value: value, expired: expired ? Date.now() + expired : 0 }
...
当我们调用setItem存储数据时，可以传入一个过期时间，比如一小时后过期
在需要读取数据的时候，可以调用getItem方法来获取数据。如果数据已经过期，就会返回null。
...
function isInViewport(element: HTMLElement): boolean {
  ...
  const elementRight = rect.width + elementLeft;
...
垃圾回收机制是指在程序运行过程中，由垃圾回收器自动释放已经不再使用的内存空间，以避免内存泄漏。
...
1. 标记清除
    ...
    JavaScript 中最常用的垃圾回收算法是标记清除算法。这种算法是基于 JavaScript 的变量作用域的概念，当一个变量不再被使用时，垃圾回收器会将它标记为无用，并清除其占用的内存空间。
    具体来说，当垃圾回收器发现某个变量不再被使用时，它会将该变量标记为“可回收”。如果这个变量占用的内存空间没有被其它变量引用，那么该内存空间就被释放。
...
2. 引用计数
    ...
    这种算法是基于内存对象的引用计数的概念，当一个对象的引用计数变为 0 时，垃圾回收器会自动清除该对象的内存空间。

    具体来说，当垃圾回收器发现某个对象的引用计数为 0 时，它会将该对象标记为“可回收”，并清除其占用的内存空间。在 JavaScript 中，引用计数算法被很少使用，主要是因为它无法解决循环引用的问题。

#### 垃圾回收的工作时机

垃圾回收器在运行时会定期检查当前程序中的变量和对象是否有无用的，然后清除它们占用的内存空间。

垃圾回收器的工作时机是不确定的，通常是在程序空闲时运行的。当然，我们也可以通过手动调用 window.gc() 方法来触发垃圾回收器的工作。

#### 垃圾回收机制的优化

为了提高垃圾回收机制的效率，JavaScript 引擎会对垃圾回收机制进行一些优化，例如：

1. 新生代对象和老生代对象分别采用不同的垃圾回收机制；
2. 对象的分代管理；
3. 垃圾回收的并行和并发。
...
1. 避免使用全局变量
    ...
    全局变量会一直存在于内存中，容易造成内存泄漏。所以我们应该尽量避免使用全局变量，使用局部变量来代替。

2. 及时清理不需要的对象

    在代码中，当我们不需要某个对象时，应该尽早将其赋值为 null，这样可以让垃圾回收器尽快回收它所占用的内存。

3. 避免循环引用

    当对象之间相互引用时，可能会出现循环引用的情况，导致垃圾回收器无法回收这些对象。所以我们要避免循环引用的情况出现。

4. 使用 let 和 const 声明变量

    使用 var 声明变量时，变量会成为全局对象的属性，容易造成内存泄漏。而使用 let 和 const 声明变量时，变量只会在块级作用域中存在，不会成为全局对象的属性。

5. 尽量避免大量使用闭包

    闭包会将外部函数的变量保存在内存中，容易造成内存泄漏。所以在使用闭包时，要注意及时释放外部函数的变量。

#### V8引擎是如何优化的

主要有两种方式：标记清除和增量标记。
标记清除是最常用的垃圾回收方式，它通过标记所有不再需要使用的对象，然后清除这些对象占用的内存。
增量标记则是一种优化的方式，它通过将垃圾回收分成多个小的步骤来避免在执行垃圾回收时阻塞JavaScript执行。

分代垃圾回收
V8将内存分为新生代和老生代两部分，新生代存放的是生命周期短的对象，老生代存放的是生命周期较长的对象。
因为新生代中对象的生命周期很短，所以它的垃圾回收更加频繁，而老生代中的对象则相对稳定，所以垃圾回收的频率也相对较低。这种方式可以有效提高垃圾回收的效率。
将堆内存分为新生代（New Space）和老生代（Old Space）两部分。新生代存储的是短生命周期的对象，老生代存储的是长生命周期的对象。当新生代内存快要用完时，V8 会执行 Scavenge 算法，将存活的对象复制到老生代中。
老生代内存达到一定阈值时，V8 会执行 Mark-Sweep & Mark-Compact 算法来清除垃圾对象。

空间复制
在新生代垃圾回收中，V8采用了空间复制的方式来优化垃圾回收。
它将新生代内存分为两个区域，每次只使用其中的一半。当一个区域被使用满后，V8会将其中还存活的对象复制到另一个未使用的区域中，同时将这个区域中的垃圾清除掉。
这种方式可以减少垃圾回收的时候需要遍历的对象数量。

增量标记
在老生代垃圾回收中，V8采用了增量标记的方式来优化垃圾回收。它将垃圾回收分成多个小的步骤，每次执行一小步之后就让JavaScript执行一会儿，然后再继续执行垃圾回收的下一小步。
这种方式可以避免垃圾回收阻塞JavaScript执行。

##### 增量标记的拆分方式

V8在增量标记的过程中，通常会将垃圾回收分为以下几个步骤：

初始标记（Initial Marking）：扫描根对象标记第一波活跃对象
在该阶段，V8会扫描程序中的根对象，并标记这些根对象以及这些根对象直接引用的对象为活动对象（也称为GC根）。这一阶段的目的是为了尽快的找出一些仍然活跃的对象，以便在后面的标记过程中排除这些已经标记的对象，从而提高标记效率。

并发标记（Concurrent Marking）：对活跃对象的引用标记为活跃对象
在该阶段，V8会扫描当前所有标记过的对象，并标记这些对象引用的对象为活动对象，然后继续并发执行程序。该阶段的目的是为了在不阻塞程序执行的情况下完成标记过程。

重新标记（Re-Marking）：再次从根对象开始记录他们引用的对象，对新的活跃对象再次标记
在该阶段，V8会再次扫描程序中的根对象，并标记这些根对象以及这些根对象直接引用的对象为活动对象。
然后，V8会扫描在并发标记阶段中发生变化的对象，标记这些对象以及这些对象引用的对象为活动对象。该阶段的目的是为了修正并发标记期间发生的变化，确保所有活跃对象都被准确的标记。

并发清除（Concurrent Sweeping）：回收所有没有标记的对象
在该阶段，V8会回收所有未标记的对象，并将这些对象的内存空间返回给堆内存池。该阶段的目的是为了在不阻塞程序执行的情况下完成清除过程。

总的来说，V8采用增量标记的方式进行垃圾回收，在程序执行的同时，逐步地标记活动对象，然后在程序执行空闲的时候完成清除过程。该方式能够充分利用空闲时间进行垃圾回收，从而减少程序因为垃圾回收而造成的停顿时间，提高程序的响应速度和执行效率。
...
词法分析（Lexical Analysis）：将 TypeScript 代码分解为词法单元（token），比如标识符、关键字、运算符、数字、字符串等等。每个词法单元都具有类型和值等属性，这些属性会用于后续的语法分析和转换过程。

语法分析（Syntax Analysis）：将词法单元按照语法规则进行组合，形成一个抽象语法树（Abstract Syntax Tree，AST）。语法分析器会根据语法规则检查代码的正确性，并将代码转换为抽象语法树。

类型检查（Type Checking）：对抽象语法树进行类型检查，检查变量和表达式的类型是否正确。

转换（Transformation）：对抽象语法树进行一些转换操作，比如将 ES6 代码转换成 ES5 代码，将 TypeScript 的类型注解转换成普通的 JavaScript 代码等等。

代码生成（Code Generation）：根据转换后的抽象语法树生成 JavaScript 代码。

TypeScript 编译器主要使用了以下技术：

Esprima：一款开源的 JavaScript 解析器，用于将 JavaScript 代码转换为抽象语法树。
TypeScript Parser：TypeScript 自己编写的解析器，用于将 TypeScript 代码转换为抽象语法树。
TypeScript Compiler API：TypeScript 提供的一组 API，用于在编译器中执行各种任务，比如类型检查、转换、代码生成等等。
Babel：一个广泛使用的 JavaScript 转换工具，可以将 ES6/ES7 代码转换为 ES5 代码，TypeScript 编译器也可以借助 Babel 进行转换。
综上，TypeScript 的编译过程包括词法分析、语法分析、类型检查、转换和代码生成等阶段，其底层原理主要是基于解析器和编译器 API 等技术实现的。
...
箭头函数表达式：可以简化函数定义的写法，同时也解决了 this 的问题
模板字符串：可以方便的在字符串中插入变量，同时也可以实现多行字符串的拼接
扩展运算符：可以将数组或对象展开成单独的参数，或将多个对象合并成一个对象
Promise：可以用来处理异步操作，解决了回调地狱的问题
类和继承：ES6 引入了 class 和 extends 关键字，使得面向对象编程更加方便
模块化：ES6 中引入了 import 和 export 关键字，可以方便的管理模块的依赖关系
Set 和 Map：可以用来保存不重复的值和键值对
Symbol：可以用来生成唯一的值，解决了属性名冲突的问题

除了这些常用的特性，还可以根据自己的经验谈一些更高级的特性，比如 Generator、async/await、Proxy 等等。不过在回答时要注意，不要夸大自己的能力，只说自己真正掌握和使用的特性即可。

## symbol类型

> cnt 1

一种基本数据类型，用于创建独一无二的标识符。它们可以用来定义对象属性的键名，以避免命名冲突，并且可以用作私有属性或方法的标识符。

### spcies的概念

Symbol.species是Symbol对象的一个内置属性，用于指定衍生对象构造函数创建的函数，通常用于定制衍生对象的行为。当衍生对象需要创建新实例时，会通过调用该属性指定的构造函数来创建实例。

Symbol.species属性通常用于可迭代对象和Promise对象的衍生类中。通过指定Symbol.species属性为自身的构造函数，可以确保衍生类的方法返回实例类型与该衍生类相同，而不是返回父类的实例类型。

例如，下面是一个用于创建可迭代对象的衍生类，其中Symbol.species属性被设置为自身的构造函数：
...
const arr2 = arr1.filter(x => x > 1); // 返回一个 MyArray 实例
console.log(arr2 instanceof MyArray); // true
```

在上面的代码中，MyArray类继承自Array类，并重写了Symbol.species属性为自身的构造函数。
当调用arr1.filter()方法过滤元素时，该方法会返回一个新的MyArray实例，而不是Array实例，因为Symbol.species属性指定了返回的实例类型。

Symbol.species属性可以让衍生类的方法返回与该衍生类相同类型的实例，这种行为有时候是非常有用的，可以让我们更加灵活地定制衍生类的行为。

### 使用

Symbol () == Symbol () 的结果是false

因为每一个都是不同的

### symbol应用场景

对象属性的键名：Symbol 值作为对象的属性名时，不会被常规方法遍历得到，从而保证了对象属性名的唯一性，避免了命名冲突。
要访问只能通过Symbol返回的属性进行获取才行

私有属性和方法：Symbol 可以作为对象的私有属性或方法的标识符，只能通过对象自身访问，外部无法访问。

迭代器：ES6 中引入了迭代器（iterator）的概念，可以用于遍历数据结构。使用 Symbol 可以定义迭代器接口。

类型判断：可以使用 Symbol.toStringTag 属性来定制对象的 toString() 方法返回的字符串。
...
在上面的例子中，\_privateMethod 被声明为 Symbol 类型，代表私有方法的标识符。
在构造函数中，通过将私有方法赋值给 \_privateMethod 来声明该私有方法，这样在外部是无法直接访问该私有方法的。
而在 publicMethod 中，可以通过 this\[\_privateMethod]() 的方式调用私有方法。
这样就可以实现私有方法的功能，同时避免了外界直接访问私有方法的情况。
...
语法更加直观和简洁：ES6 的类引入了 class 关键字来定义一个类，使得类的定义更加直观和简洁，同时也让 JavaScript 类和其他面向对象语言的类定义方式更加一致。

支持类的继承：ES6 的类支持了继承，通过 extends 关键字可以方便地实现类的继承，同时也支持 super 关键字来访问父类的属性和方法。

支持定义静态方法和属性：ES6 的类支持在类中定义静态方法和属性，通过 static 关键字可以方便地定义只属于类本身而不是实例的方法和属性。

更好的封装性和作用域控制：ES6 的类支持使用 constructor 方法来定义构造函数，并且可以使用 let 或 const 关键字来声明私有变量，从而实现更好的封装性和作用域控制。

更好的性能和代码可读性：ES6 的类相比 ES5 的构造函数，在一些场景下具有更好的性能，同时也让代码更加可读易懂。

### 设计私有属性的方法

1. private或#
编译器会在每个私有字段的前面添加一个类似于 _className 的前缀，其中 className 是类的名称，这个前缀是由编译器生成的，可以避免与其他字段的名称冲突。这个前缀是在编译时进行处理的，因此在运行时无法访问它。
2. weakMap
WeakMap，它是一种弱引用类型的Map，它的key只能是对象，而且这些对象是弱引用，即如果这些对象在代码中没有被引用，那么它们会被垃圾回收器回收。这就保证了对象的私有属性只能通过这个对象本身访问，而外部无法获取到这个WeakMap实例，也就无法获取到私有属性的值。同时，由于WeakMap的key是弱引用的，如果对象被回收了，那么这个对象对应的私有属性也会被自动清除，不会造成内存泄漏。
3. 闭包
闭包则是通过函数作用域的特性来实现私有属性的封装。在一个函数内部定义的变量，在函数执行完后就会被销毁，而外部无法访问这个变量。如果我们将一个函数内部定义的变量作为对象的私有属性，那么这个变量就只能被这个对象内部的函数所访问和修改，而外界无法获取和修改这个私有属性的值。
4. symbol
...
类似于继承中的super，可以对代理的对象本身进行内容的获取
...
微前端是一种架构模式，它将前端应用程序拆分成更小、更独立的部分，这些部分可以被不同的团队独立开发、测试、部署和扩展。每个小部分被称为微前端，它们可以独立部署和运行，并且可以通过远程调用、消息传递等方式与其他微前端通信。

微前端旨在解决大型前端应用程序开发和维护过程中的一些常见问题，例如：

复杂性：大型前端应用程序往往会变得非常复杂，难以维护和扩展。微前端将应用程序拆分成更小的部分，使得每个部分都相对简单，易于开发和维护。

团队协作：大型前端应用程序需要多个团队合作开发，不同团队之间往往存在协作问题。微前端可以将应用程序拆分成更小的部分，每个部分可以由不同的团队独立开发，大大减少了团队之间的协作难度。

技术栈：大型前端应用程序往往使用多种技术栈开发，不同技术栈之间往往存在集成问题。微前端可以将应用程序拆分成更小的部分，每个部分可以使用不同的技术栈开发，使得整个应用程序可以更容易地集成不同的技术栈。

性能：大型前端应用程序往往需要加载大量的代码和资源，影响性能。微前端可以将应用程序拆分成更小的部分，每个部分可以被独立加载，从而提高了应用程序的性能。

### 微前端的弊端

复杂性：微前端需要管理多个独立的应用程序，因此在一些方面会变得更加复杂。例如，微前端需要解决应用程序之间的通信问题、版本管理问题等。

安全性：微前端需要通过远程调用或消息传递等方式与其他微前端通信，因此需要考虑安全性问题。

性能：微前端需要加载多个独立的应用程序，因此可能会增加一些性能开销。

总之，微前端是一种有利于大型前端应用程序开发和维护的架构模式，但需要谨慎应用，权衡其优缺点，根据实际情况进行选择。

### 淘宝如何使用微前端的

一个使用了微前端架构的例子是淘宝的“淘宝客户端 7.0”。该应用使用微前端架构来实现多团队协同开发和部署。具体来说，该应用被拆分为多个微前端应用，每个微前端应用都是独立开发、测试、部署的，它们之间通过共享的接口和通信机制进行交互。

例如，淘宝客户端包含了多个业务场景，如首页、我的淘宝、购物车、商品详情等，每个场景都对应着一个独立的微前端应用。这些微前端应用通过路由机制进行整合，从而形成了一个完整的客户端应用。每个微前端应用都可以独立运行、开发和部署，它们之间的依赖关系也被限制在了共享接口的范围内，避免了耦合和影响范围扩大的问题。

通过微前端架构，淘宝客户端可以实现多团队协同开发和部署，每个团队都可以独立开发和测试自己的微前端应用，不会对其他团队产生干扰。同时，微前端架构也可以提高系统的可维护性和可扩展性，通过动态加载和卸载微前端应用，可以实现更加灵活的部署和升级策略。然而，微前端架构也存在一些弊端，例如增加了系统的复杂度，需要额外的技术支持和管理工作。
...
##### 初始化一个vue项目需要哪些loader和plugin

可以使用Vue CLI工具来初始化项目。在初始化过程中，Vue CLI会默认配置好一些必要的loader和plugin

vue-loader：用于解析.vue单文件组件，同时负责热更新（为了支持.vue文件中的样式热更新，vue-loader还需要配合另外一个插件vue-style-loader使用。vue-style-loader会将编译后的样式代码动态插入到页面中，从而实现样式的热更新。）
babel-loader：用于将ES6+的代码转换成ES5的代码，以便浏览器能够正确地运行。
css-loader和style-loader：用于解析和处理CSS代码。
file-loader和url-loader：用于处理图片、字体等静态资源文件。
vue-template-compiler和html-webpack-plugin：用于将Vue单文件组件中的模板编译成HTML，并将生成的HTML文件自动注入到HTML模板中。

##### 初始化一个react项目需要哪些loader和plugin

少了vue有关的，多了react的热更新loader

### 打包结构

主要包含以下内容：

bundle.js：这是包含所有模块的 JavaScript 文件，也是打包后的主要文件。
chunk.js：这是 Webpack 分割代码块时生成的 JavaScript 文件，用于按需加载。
index.html：这是应用的入口 HTML 文件。
assets：这是包含所有打包后资源文件（如图片、字体、样式等）的文件夹。
以下是一个更详细的树状结构：
...
assets/ 目录下包含了打包后的所有静态资源。
bundle.js 是打包后的主要 JavaScript 文件，包含所有的模块和依赖。
chunk-1.js 和 chunk-2.js 是 Webpack 根据代码分割策略分割出来的代码块，这些代码块可以按需加载。
index.html 是应用的入口 HTML 文件，用于引入打包后的 JavaScript 和 CSS 文件。
assets/styles/main.css 是打包后的主要 CSS 文件，包含了所有模块的样式。
assets/styles/main.css.map 是主要 CSS 文件的 Source Map 文件，用于开发环境调试。
在开发环境中，Webpack 还会生成其他文件，如：

bundle.js.map 是打包后 JavaScript 文件的 Source Map 文件，用于开发环境调试。
index.html 是用于开发服务器的 HTML 文件，用于自动注入打包后的 JavaScript 和 CSS 文件。

其他文件：Webpack 除了以上几个部分之外，还可能生成一些其他的文件，例如缓存文件、日志文件等。
...
├── js/
│   ├── app.[hash].js
│   ├── app.[hash].js.LICENSE.txt
│   ├── chunk-vendors.[hash].js
│   └── chunk-vendors.[hash].js.LICENSE.txt
├── favicon.ico
├── index.html
└── assets/
    ├── img/
...
css/：存放所有的 CSS 文件。
js/：存放所有的 JavaScript 文件。
favicon.ico：网站图标。
index.html：打包后的首页。
assets/：存放静态资源，如图片、字体等。
在 css/ 目录下，app.\[hash].css 是应用程序的 CSS 文件，而 chunk-vendors.\[hash].css 包含了第三方库的 CSS 文件。
在 js/ 目录下，app.\[hash].js 是应用程序的 JavaScript 文件，chunk-vendors.\[hash].js 包含了第三方库的 JavaScript 文件。.LICENSE.txt 文件包含了相关的授权信息。
在 assets/ 目录下，存放所有的静态资源文件。

在 Vue 项目中，可以通过配置实现将多个 CSS 或 JS 文件合并成一个文件，从而减少 HTTP 请求，提高页面加载速度。

为什么一开始不合并：

增量更新：将JS和CSS文件拆分成多个小文件，可以利用浏览器缓存机制，减少重复下载，提高加载速度，并且当只有某个模块发生变化时，只需要重新下载该模块对应的JS或CSS文件，而不需要重新下载整个文件。

并行下载：将JS和CSS文件拆分成多个小文件，可以同时在多个线程中下载，提高加载速度。

代码分离：将JS和CSS文件拆分成多个小文件，可以根据不同的路由或组件动态加载不同的文件，实现代码分离，降低首屏加载时间。

虽然将JS和CSS文件拆分成多个小文件有助于提高网页加载速度，但如果文件数量过多，也会增加HTTP请求次数，反而会降低网页加载速度，因此需要根据具体情况进行合理的文件拆分和合并。

#### 抽取频繁使用的代码

要将频繁使用的js文件抽取出来，可以使用Webpack的SplitChunksPlugin插件，将公共模块抽离成单独的文件，以减少每个页面需要加载的代码量。具体的配置可以在Webpack的配置文件中进行设置。

通常情况下，抽取出来的公共 JS 代码会放在 vendor 中。但是具体实现可以根据配置和插件不同而有所不同
...
Webpack 会通过监听文件系统的方式来检测代码变化。当一个模块的代码发生变化后，Webpack 会重新构建该模块以及依赖它的其他模块，并将构建后的代码发送给浏览器。为了实现这个功能，Webpack 使用了一个叫做 "watch mode" 的模式，它会在后台持续监听文件系统的变化。当有文件发生变化时，Webpack 会重新构建模块并通知 Webpack Dev Server，WebDevServer 再通过 WebSocket 向浏览器发送更新通知，实现了热更新的功能。
...
Webpack打包优化性能的策略有很多，以下是一些常用的策略：

开启Tree Shaking：通过Tree Shaking可以消除无用代码，减少打包文件的大小。在Webpack中可以通过在配置文件中设置mode: 'production'来开启Tree Shaking。

使用code splitting：通过code splitting可以将代码切割成多个bundle，使得页面加载速度更快。在Webpack中可以使用SplitChunksPlugin插件来进行code splitting。

使用按需加载（lazy loading）：通过按需加载可以减少页面的初始加载时间，提高用户体验。在Vue项目中可以使用vue-router提供的import()方法进行按需加载。

压缩代码：通过压缩代码可以减小打包后的文件体积，提高页面加载速度。在Webpack中可以使用UglifyJsPlugin和OptimizeCssAssetsPlugin等插件来进行代码压缩。

使用缓存：通过使用缓存可以减少重复打包的时间，提高构建速度。在Webpack中可以使用cache-loader和hard-source-webpack-plugin等插件来进行缓存。
...
对于一个基于Vue2的组件库项目，可以考虑以下优化策略：

使用动态导入（dynamic import）来异步加载组件，提升页面加载速度。

针对生产环境，启用minification和uglify等插件来压缩JavaScript和CSS文件。

使用CDN等方式来提升静态资源加载速度。

避免在组件中直接引用第三方库，以避免将不必要的代码打入组件中。

使用babel-loader对ES6+语法进行转换，以兼容更多浏览器。

使用Vue Loader来加载Vue组件，并使用vue-template-compiler将Vue组件编译成渲染函数。

使用webpack-bundle-analyzer来分析打包后的文件，找出打包体积过大的模块，并进行优化。

配置alias来简化导入路径，提高开发效率。

在output配置中使用filename: '\[name].min.js'来生成压缩后的文件名。

综上所述，以上是一些常用的Webpack打包优化性能的策略，对于基于Vue2的组件库项目可以根据实际情况进行选择和调整。
...
1. 模块处理方式
Webpack适用于处理复杂的模块依赖关系，支持多种类型的模块格式，例如 CommonJS、AMD、ES6等；而Rollup则适用于处理简单的模块依赖关系，主要针对ES6模块格式。

2. 打包方式
Webpack通过将所有代码打包进一个或多个bundle.js文件来处理代码的打包，而Rollup则采用一种类似于“tree shaking”的方式，将未被使用的代码删除后生成较小的打包文件。

3. 插件生态系统
Webpack拥有更丰富的插件生态系统，因为它可以处理更复杂的场景。而Rollup的插件生态系统相对较小，因为它专注于处理简单的场景。
...
使用了 Vue 自己实现的一套模块系统，这套模块系统会在每个组件被加载时为其创建一个模块上下文（module context），然后在模块上下文中运行组件的代码。在热更新时，这个模块上下文会被更新，然后重新运行组件的代码。与 ES6 模块相比，Vue 2 的模块系统更加灵活，但也更加复杂。而 Vue 3 则采用了基于 ES6 模块的热更新机制，这使得热更新变得更加简单高效。

Vite 通过解析模块依赖关系，将每个模块转换成一个 ESM（ES6 模块），并在服务器上运行一个开发时的 HTTP 服务器。当我们修改了某个模块时，Vite 会重新生成该模块的 ESM，通过浏览器端的 WebSocket 将模块的修改推送给浏览器，从而实现即时热更新。

这个过程与 Webpack 和vue2 不同，Webpack 会将所有模块打包成一个或多个 bundle，通过浏览器端的 HMR（Hot Module Replacement）插件来实现即时热更新。

#### vite通知浏览器

Vite 通过 HTTP 服务器（例如 Koa）提供了一个开发服务器，在收到浏览器请求变化的模块代码的请求后，会动态地编译和转换相应的模块，并返回给浏览器。
这个过程中，由于 Vite 编译和转换过程采用了基于模块的热更新机制，所以只有改变的模块会被重新编译和转换，而不是重新构建整个应用程序。这也是 Vite 极大提高开发效率的重要原因之一。

当浏览器获取到变化的模块代码后，会进行一系列处理和更新，最终将更新的内容展示在页面上。具体来说，浏览器会通过 Vue.js 的渲染函数对组件进行重新渲染，然后将更新的 DOM 元素插入到页面中，实现即时热更新的效果。
...
一个webpack的loader，它的作用是将ES6及以上版本的JavaScript代码转换为向下兼容的ES5代码，以便让不支持新语法的浏览器或环境可以正常运行。下面是babel-loader的具体处理流程：

读取待处理的JavaScript源码；

1. 将源码转化为抽象语法树（AST）；
2. 对AST进行遍历和转换，将ES6+的语法转化为ES5的语法，比如箭头函数、类、const/let等；
3. 根据插件的配置，在转换过程中可能会对源代码进行一些额外的操作，比如添加兼容性垫片、注入运行时helper函数等；
4. 将转换后的代码重新生成为JavaScript源码；
5. 将生成的JavaScript源码作为模块返回给webpack，以便后续的模块打包和处理。

需要注意的是，babel-loader只是babel工具链中的一环，它实际上是通过调用@babel/core等其他组件来实现JavaScript源码的转换的。因此，在使用babel-loader时，需要安装和配置好babel的其他组件和插件，以便能够顺利地完成代码的转换。
...
在 ES6 使用不当时，会由 js 引擎或 js 运行时的适配层跑出错误，即不通过 throw，而是直接引发的错误
...
ES6 模块有 5 种状态，分别为 unlinked、linking、linked、evaluating 和 evaluated，用循环模块记录（Module Environment Records）的 Status 字段表示。
ES6 模块的处理包括连接（link）和评估（evaluate）两步。连接成功之后才能进行评估。

连接主要由函数 InnerModuleLinking 实现。
函数 InnerModuleLinking 会调用函数 InitializeEnvironment，该函数会初始化模块的环境记录（Environment Records），主要包括创建模块的执行上下文（Execution Contexts）、给导入模块变量创建绑定并初始化为子模块的对应变量，给 var 变量创建绑定并初始化为 undefined、给函数声明变量创建绑定并初始化为函数体的实例化值、给其他变量创建绑定但不进行初始化。

评估主要由函数 InnerModuleEvaluation 实现。
函数 InnerModuleEvaluation 会调用函数 ExecuteModule，该函数会评估模块代码（evaluating module.\[\[ECMAScriptCode]]）。ES6 规范并没有明确说明这里的评估模块代码具体指什么。

我把 ES6 规范的相关部分反复看了至少十余遍，才得出一个比较合理的解释。
这里的评估模块代码应该指根据代码语句顺序执行条款 13、条款 14 和条款 15 内的对应小节的“运行时语义：评估（Runtime Semantics: Evaluation）”。ScriptEvaluation 中的评估脚本（evaluating scriptBody）应该也是这个意思。可以看到，ES6 规范虽然做了很多设计并且逻辑清晰和自洽，但仍有一些模棱两可的地方，没有达到一种绝对完善和无懈可击的状态。
...
2. CommonJS 模块是运行时加载，ES6 模块是编译时输出接口。
    ...
    这里不完全正确，common 确实是执行阶段加载，es6 在预处理时加载，但 es6 也会在执行过程中加载（即去获取，而不是），只是此时使用的是预处理时加载的缓存
...
TypeScript 工程的循环引用问题是比较普遍的，常常会因为需要使用一个类型而增加一个文件依赖。
建议在工程中引入模块循环引用检测机制，比如 webpack 插件 circular-dependency-plugin 和 eslint 规则 import/no-cycle，以便及时调整文件或代码结构来切断循环引用。
...
Axios拦截器是一种机制，可以在发送请求或响应返回之前，对请求或响应进行拦截并做相应的处理。拦截器由请求拦截器和响应拦截器组成。

在Axios中，我们可以使用axios.interceptors.request.use方法添加请求拦截器，使用axios.interceptors.response.use方法添加响应拦截器。拦截器会返回一个拦截器ID，可以使用axios.interceptors.request.eject(id)和axios.interceptors.response.eject(id)方法来移除相应的拦截器。

拦截器可以用来进行一些公共的操作，比如添加请求头、设置超时时间、请求加密等等。
另外，拦截器还可以用来取消请求，例如在某些条件下，我们不再需要响应请求，可以使用axios.CancelToken来创建一个取消请求的Token，将其传递给请求的cancelToken参数中，然后在拦截器中调用cancel方法来取消请求。
...
在上面的例子中，我们添加了一个请求拦截器，在请求头中添加了一个Authorization字段；
同时，我们也添加了一个响应拦截器，在处理响应数据之前进行了一些操作。
最后，我们通过创建CancelToken来取消请求，拦截器会捕获到取消请求的异常并进行处理。

#### 如何取消请求

在发起 axios 请求后，将其返回的 cancelToken 存储到变量中，并通过执行 source.cancel() 方法来取消请求。
这是因为 axios 的取消请求功能是基于 Axios Cancel Token 实现的。

Axios Cancel Token 是一个通过工厂方法创建的取消令牌对象，当调用 cancel 方法时，会将 Cancel Token 中的 flag 设置为 true，这样就可以通过检查 flag 来判断请求是否被取消。

在代码中，我们可以通过创建一个 Cancel Token 和对应的 Cancel 对象，并将其作为配置参数传递给 axios，从而在请求发送后通过调用 Cancel 对象的 cancel 方法来取消请求。同时，我们还可以将 Cancel Token 存储到变量中，以便在需要取消请求时调用 cancel 方法。

在 Axios 内部，当一个请求被取消后，会将该请求对应的 CancelToken 实例的 promise 状态设置为 resolved，并将错误信息设置为 Cancel 类型的错误，这个错误包含了一个 message 属性，我们可以通过判断这个 message 是否是 Cancel 类型的错误，来判断请求是否被取消。

#### fetch类型如何实现取消

在使用 axios 取消请求时，是基于浏览器原生提供的 XMLHttpRequest 对象实现的。而 fetch 使用的是浏览器原生提供的 fetch 函数，它的取消操作需要借助于 AbortController 和 AbortSignal。

不可以直接通过 Axios 取消 Fetch 请求，因为 Fetch 请求和 Axios 请求是不同的两种网络请求库，它们之间的取消机制是不兼容的。

通过发起请求时将AbortController的实体传给fetch，便可通过abort方法取消
...
具体来说，服务端渲染会在服务器端执行组件的渲染，并生成一个已经包含了数据的 HTML 字符串返回给浏览器，而不是像传统的客户端渲染方式那样，先返回一个空的 HTML 页面，然后在浏览器端通过 JavaScript 执行组件的渲染。这种方式能够避免客户端渲染中因为需要等待 JavaScript 加载和执行而产生的页面空白时间，从而提升用户体验。

当然，并不是所有的应用都适合使用服务端渲染，因为服务端渲染也有一些缺点，比如在开发和维护上更加复杂，因为需要同时处理服务器端和客户端的逻辑，而且也不太适合一些动态交互性比较强的应用。因此，使用服务端渲染需要根据具体的业务场景和需求来进行权衡和选择。

虽然服务端渲染能够加快首次渲染速度，但是如果服务器性能不够好，在高并发的情况下服务端渲染也可能变得更慢。因为服务端需要处理更多的请求，消耗更多的资源，如果服务器负载过高可能会导致响应变慢或崩溃。

### 解决的问题

1. seo

    搜索引擎优先爬取页面HTML结构，使用ssr时，服务端已经生成了和业务关联的HTML，有利于seo
    索引擎爬取页面时，只能获取到 HTML、CSS 和 JavaScript 等静态资源，而客户端渲染的页面需要在浏览器中执行 JavaScript，才能获取到动态内容，这样搜索引擎无法获取到这些动态内容中的关键词，对 SEO 不友好。
    而 SSR 在服务器端已经将页面渲染完成，生成了完整的 HTML 页面，搜索引擎可以直接获取到其中的所有内容和关键词，因此有利于提高页面在搜索引擎中的排名。

2. 首屏呈现渲染

    用户无需等待页面所有js加载完成就可以看到页面视图（压力来到了服务器，所以需要权衡哪些用服务端渲染，哪些交给客户端）
...
页面加载时间（Page Load Time）：从发起页面请求到页面内容完全加载完成的时间，包括 HTML、CSS、JavaScript、图片等所有资源的加载时间。
首次渲染时间（First Paint Time）：从页面开始加载到浏览器开始将内容渲染到屏幕上的时间。
首次内容渲染时间（First Contentful Paint）：从页面开始加载到浏览器开始渲染第一个页面内容的时间。
可交互时间（Time to Interactive）：从页面开始加载到用户可以与页面上的元素进行交互的时间。

用户体验性能指标：

页面滚动性能（Scrolling Performance）：页面滚动时的流畅度和响应速度。
页面动画性能（Animation Performance）：页面中的动画效果的流畅度和响应速度。
用户输入响应时间（Input Response Time）：用户在页面上输入操作的响应速度。

资源利用率指标：

资源压缩率（Resource Compression）：CSS、JavaScript、图片等资源的压缩比例。
资源合并率（Resource Concatenation）：CSS、JavaScript 等资源的合并比例。
资源缓存率（Resource Caching）：静态资源的缓存比例。

计算这些性能指标需要使用一些工具和技术，例如：

使用性能监测工具：

Lighthouse：Lighthouse 可以检测网页的性能、可访问性、最佳实践和 SEO 等方面，并提供建议和改进措施。
WebPageTest：WebPageTest 可以测试网页的加载速度、资源利用率、响应时间、重定向次数等方面，并提供详细的性能数据和图表。

使用浏览器开发者工具：

Network 面板：可以查看网页加载的资源、大小、加载时间等信息。
Performance 面板：可以记录并分析网页的性能指标，例如加载时间、渲染时间、CPU 使用率等。

使用代码优化技巧：

压缩和合并资源文件，减少 HTTP 请求次数。
避免使用大量的第三方库和插件。
使用 Web Workers 和 Service Workers 等技术，提高页面的并发处理能力。
使用图片懒加载、无限滚动等技术，减少页面资源的加载时间。

总之，页面性能是一个综合性的指标，需要从多个方面综合考虑和优化，才能达到更好的用户体验。
...
协议（Protocol）：指定通信协议的名称，通常是HTTP或HTTPS，以及其他协议如FTP、SMTP等。
域名（Host）：指定服务器的域名或IP地址。
端口号（Port）：指定服务器监听的端口号，一般情况下HTTP服务使用80端口，HTTPS服务使用443端口。
路径（Path）：指定服务器上请求资源的路径和名称。
查询字符串（Query String）：以“?”开头，用于向服务器传递参数。
片段标识符（Fragment Identifier）：以“#”开头，用于指定文档中某个位置的标识符。

其中，协议、域名、端口号、路径和查询字符串合称为URL的五个组成部分，片段标识符不属于URL，而是在浏览器中用于标记文档的特定位置。
...
与 requestAnimationFrame 功能类似的 API 是 setTimeout 和 setInterval，它们也可以用于执行指定的函数，但它们的执行时机不同。
setTimeout 和 setInterval 是定时器 API，它们会在指定的时间间隔之后执行函数，而不考虑浏览器的重绘时机。相比之下，requestAnimationFrame 会在浏览器准备进行下一次重绘时执行函数，这样可以确保函数的执行时机与浏览器的重绘时机保持一致，从而实现更加流畅的动画效果。
...
焦点事件（focus、blur）
鼠标滚轮事件（mousewheel）
事件执行的优先级是根据事件的类型不同而不同的。一般来说，用户交互类事件（例如点击、鼠标移动等）的优先级会比较高，而其他事件（例如加载完成、动画结束等）的优先级会比较低。如果多个事件同时触发，执行的优先级是根据事件注册的先后顺序来决定的。

### 移动端的事件

移动端触屏设备的事件与桌面端的鼠标事件不同，例如touchstart、touchmove、touchend、touchcancel事件取代了mousedown、mousemove、mouseup等鼠标事件。

由于移动设备的硬件限制，有些事件无法在移动端触发。例如，移动端的浏览器不支持mousemove事件，因为没有鼠标指针。另外，resize事件也不同于桌面端，因为移动端设备的屏幕大小不会随意改变，故resize事件在移动端可能不会被触发。

需要注意的是，移动端触屏事件和手势事件的触发顺序和桌面端的鼠标事件有所不同。
在移动端上，touchstart事件通常优先于mousedown事件触发；
而在触发手势事件时，则需要考虑手势事件的先后顺序，例如tap事件会优先触发，然后再依次触发swipe、pinch等手势事件。

#### touch事件

移动端特有的事件，主要用于监听用户在触摸屏幕上的手指移动、放开、滑动等操作。

tap是一种常用的手势操作，是通过连续的 touchstart、touchend 事件模拟实现的。当用户在屏幕上单击时，会触发一次 tap 事件。一般来说，tap 操作优先级比较低，因为它需要等待手指离开屏幕后才会触发，而用户在点击时需要等待一定的时间才能得到响应，因此在用户体验上可能会稍有延迟。

##### 手势事件

同时，移动端还新增了一些手势事件，如tap、longTap、swipe等，用于支持更加友好的移动端交互体验。
Tap（点击）：轻触触摸屏幕。
Double tap（双击）：双击触摸屏幕。
Long press（长按）：长时间按住触摸屏幕。
Swipe（滑动）：快速滑动触摸屏幕。
Flick（轻扫）：快速滑动触摸屏幕，然后立即松开。
Pinch（缩放）：用两个手指在屏幕上同时向内或向外移动，改变元素的大小。
Rotate（旋转）：用两个手指在屏幕上同时旋转，改变元素的方向或角度。
...
同源策略：浏览器只允许与同域名、同协议、同端口的网站进行交互，不允许跨域访问其他网站的数据。如果需要跨域访问其他网站的数据，需要使用CORS等技术来进行授权访问。

Cookie安全策略：Cookie是一种存储在客户端的数据，可以记录用户的登录状态等信息，但是Cookie也有被窃取的风险。浏览器通过限制Cookie的作用域、有效期等方式来保障Cookie的安全。

HTTPS：HTTPS是一种通过TLS/SSL加密通信的协议，可以保障用户数据在传输过程中不被窃取、篡改等风险。

CSP：CSP（Content Security Policy）是一种安全策略，可以限制网站资源（如脚本、图片等）的来源，从而防止恶意脚本的注入攻击。
用于限制页面中可以执行的代码的来源，从而降低恶意攻击的风险。通过 CSP，网站开发人员可以指定哪些类型的资源可以被加载，例如 JavaScript、CSS、图片等，并且指定这些资源可以从哪些来源加载，以及是否允许执行内联脚本和样式。
CSP通过使用HTTP头来实现。开发人员可以在HTTP响应头中添加Content-Security-Policy头来定义策略，也可以使用X-Content-Security-Policy、X-WebKit-CSP等其他头部来定义策略。

除了以上策略，还有一些其他的安全策略，如XSS（跨站脚本攻击）防御、CSRF（跨站请求伪造）防御等，都是为了保障用户在浏览器中的安全。
...
抽象底层框架：跨端框架通常会抽象出一套通用的底层框架，用于处理不同平台的基本操作，例如网络请求、文件系统、存储等。这些底层框架通常会使用平台特定的 API 或插件来实现。

跨平台视图渲染：跨端框架通常会使用一套统一的视图渲染引擎，将视图的布局和样式信息转换成不同平台的代码。这通常需要针对不同平台的特性和限制，编写特定的渲染代码和布局算法。

组件化开发：跨端框架通常会使用组件化的开发方式，将业务逻辑和视图渲染封装在独立的组件中，以便在不同平台上重用和定制。组件通常需要编写平台特定的代码和样式，以便适应不同的平台特性和限制。

构建打包：跨端框架通常需要使用特定的工具链来构建和打包应用程序，将跨平台代码转换为特定平台的代码，并将应用程序打包成特定平台的格式。这通常需要使用不同平台的编译器、打包工具和调试器等。

### 基于JSCore转化其他平台

解析 JSX 代码
跨端框架的编译器会先将 JSX 代码解析成虚拟 DOM 对象，即将代码转换成 JavaScript 对象，包括节点类型、属性、子节点等信息。

生成抽象语法树
将解析后的虚拟 DOM 对象转换成抽象语法树（AST），也就是用纯 JavaScript 对象描述 JSX 代码的结构。

生成中间代码
根据抽象语法树生成中间代码，即通过一定的规则将抽象语法树转换为平台无关的中间代码。这里需要考虑到不同端的差异性，例如 Android 和 iOS 平台的差异，需要针对不同的端生成不同的中间代码。

生成目标代码
根据生成的中间代码，再针对不同的端生成目标代码，即生成真正的原生组件代码。生成的目标代码可以通过运行时库实现渲染。

注入 JavaScript 运行环境
将生成的目标代码注入到目标端的 JavaScript 运行环境中，即运行在 JavaScriptCore 中的代码，使其可以在目标端上运行并渲染。

与 iOS 平台上不同的是，安卓平台上并没有内置像 JSCore 这样的 JavaScript 引擎，因此这些框架通常会集成一个 JavaScript 引擎，例如 V8 引擎，用于解析和执行 JavaScript 代码。

具体来说，跨端框架一般会将 JavaScript 代码转换为中间代码，并在运行时解析和执行这些中间代码。在解析和执行过程中，使用的是 JavaScript 引擎提供的 API。通过这些 API，跨端框架可以访问原生组件，并进行原生组件的创建、渲染等操作。

总之，安卓平台上的跨端框架通常会集成一个 JavaScript 引擎，例如 V8 引擎，用于解析和执行 JavaScript 代码，从而实现跨端的效果。

#### JSCore的作用

JSCore 扮演了至关重要的角色。JSCore 是 iOS 平台上的 JavaScript 引擎，它可以将 JavaScript 代码转换为可以在 iOS 平台上运行的二进制代码，同时也可以与原生代码进行交互。

在跨端框架的跨端转换过程中，JSCore 主要起到两个作用：

解析执行 JavaScript 代码
跨端框架将通过 JSX 编写的 JavaScript 代码转换为中间代码，这些中间代码实际上是一系列的 JavaScript 代码。然后，这些中间代码将被注入到 JSCore 中，并由 JSCore 解析执行。

与原生代码进行交互
跨端框架生成的目标代码是原生组件代码，需要与原生代码进行交互。这时，跨端框架会将一些 JavaScript 函数或对象暴露给原生代码，原生代码就可以通过 JSCore 调用这些函数或对象。

总之，JSCore 作为 iOS 平台上的 JavaScript 引擎，在跨端框架的跨端转换过程中，扮演着非常重要的角色，能够解析执行 JavaScript 代码，并与原生代码进行交互，实现跨端的效果。
...
通过URL Scheme（统一资源定位符方案）实现从网页跳转到原生应用，或者从原生应用跳转到网页。例如，通过链接<\a href="weixin://">打开微信</、a>可以跳转到微信原生应用，通过JavaScript代码window.location.href="weixin://"也可以实现跳转。
淘宝的深度链接示例：

```html
<a href="taobao://detail?id=123456">打开淘宝商品详情页</a>
```

用户点击该链接后，如果已经安装了淘宝APP，就可以打开淘宝APP，并跳转到指定商品详情页。

### 桥接jsBridge

通过JavaScript和原生应用之间的桥接实现交互。例如，可以通过JavaScript代码调用原生应用提供的API，或者原生应用调用JavaScript代码提供的方法，实现数据的传递和处理。

在APP中，通过WebView中的JavaScript Bridge，可以调用Native提供的API。比如，我们可以在H5页面中定义以下JavaScript代码

```js
window.JsBridge.invoke('login', {
...
这段代码调用了Native提供的登录接口。当用户点击登录按钮时，就会触发这段代码，并向Native发送登录请求。Native在处理完成后，可以调用JavaScript中定义的success或fail回调函数，通知H5页面登录的结果。

### webview

在原生应用中嵌入网页，通过Webview实现网页和原生应用的交互。例如，可以在一个原生页面中嵌入网页，通过Webview控件加载网页内容，并在JavaScript代码中与原生应用进行交互。

在APP中，一般会内置一个WebView，用来加载H5页面。当需要跳转到H5页面时，只需要在WebView中加载对应的URL即可。

### 通用链接

通过在淘宝App中关联域名 taobao.com，当用户在App中安装了淘宝App后，点击网页上的链接 https://item.taobao.com/item.htm?id=xxxxxx 可以直接打开淘宝App的商品详情页，无需提示选择应用。

淘宝的通用链接示例：

```html
<a href="https://s.click.taobao.com/abcde">打开淘宝商品详情页</a>
<link rel="apple-app-site-association" href="https://www.taobao.com/apple-app-site-association">
```

当用户点击该链接后，系统会先检查是否安装了淘宝APP，并检查淘宝APP是否支持这个通用链接，如果支持，就会打开淘宝APP，并跳转到指定商品详情页。
...
通过HTTP报文的哪个字段设置cookie，有一个关键字

发送请求时需要设置withCredentials属性为true,来允许浏览器在自己的域设置cookie值

```java
response.setHeader('Access-Control-Allow-Credentials',true);
response.setHeader('Access-Control-Allow-0rigin','http://localhost:63342');
...
HTTPS是HTTP协议和SSL/TLS加密协议的组合，通过使用SSL/TLS协议对HTTP请求和响应数据进行加密和解密，提供了数据传输的安全性。

HTTPS则通过使用SSL/TLS协议对HTTP请求和响应数据进行加密和解密，提供了数据传输的安全性，具体包括：
加密：HTTPS使用SSL/TLS协议对HTTP数据进行加密，防止数据在传输过程中被窃取或篡改。
身份验证：HTTPS使用证书来验证服务器和客户端的身份，防止中间人攻击。

#### 为什么有时候会更慢

HTTPS相对于HTTP来说，因为要进行加密解密操作，可能会带来一些性能上的损失。具体来说，HTTPS相对于HTTP会增加以下几个方面的开销：

加密解密操作：HTTPS需要进行加密解密操作，对于服务器和客户端都会带来一定的开销，尤其是对于一些低配置的服务器来说，这个开销可能会比较大。

握手时间：在HTTPS连接建立时，需要进行一次握手过程，这个过程中需要客户端和服务器进行多次数据传输和计算，因此会增加一定的延迟时间。

证书校验：HTTPS连接需要进行证书验证，这个过程需要进行一些网络通信和加密解密操作，也会增加一定的延迟时间。

缓存问题：HTTPS连接中的数据通常不能被缓存，因为每个请求和响应都是加密的，所以需要每次重新获取数据，这也会带来一定的延迟时间。

总之，HTTPS相对于HTTP来说，在一定程度上会增加一些性能上的开销，但是为了网络安全和数据隐私，使用HTTPS是非常必要的。如果你的网站响应速度较慢，可以考虑对HTTPS的一些优化措施，例如使用CDN加速、使用更快的加密算法等等。
...
JSONP的实现原理是利用HTML的\<\script>标签可以跨域访问资源的特性，通过动态创建\<\script>标签，向目标服务器发出GET请求，服务器返回一段包裹在函数调用中的JSON格式数据，客户端通过解析JSON数据并执行包裹的函数来获取数据。

例如，客户端需要请求一个跨域的数据接口：
<!-- http://example.com/data?callback=handleData -->
其中，callback参数指定了回调函数的名称，例如这里指定的是handleData。服务器返回的数据如下：
<!-- handleData({"name":"John", "age":30, "city":"New York"}) -->
客户端动态创建一个\<\script>标签，引用上述URL：
<!-- <script src="http://example.com/data?callback=handleData"></、script> -->
此时服务器返回的数据会被包裹在handleData()函数中，客户端浏览器接收到响应后，会自动执行该函数，并将解析得到的JSON数据传递给该函数作为参数，客户端就可以获取到跨域的数据。

需要注意的是，JSONP的实现需要服务器端配合，在返回数据时需要将数据包裹在指定的回调函数中，因此只有服务器端支持JSONP，才可以在客户端使用JSONP进行跨域数据访问。
...
Access-Crontrol-Allow-Origin（来源地址）,Headers（头部字段）,Method（方法）

头部的作用：
可能会不经意的发出敏感字段,例如“Authorization”头部，这可能导致安全问题。
此外，允许所有的headers也会增加跨站脚本攻击（XSS）的风险，因为攻击者可以使用自定义的headers来欺骗服务器。
...
在网页中生成二维码。可以使用现成的二维码生成库，例如qrcode.js。将生成的二维码展示在网页上。

在网页中轮询是否已经扫码。可以使用Ajax轮询方式，每隔一段时间向后台发送请求，询问是否已经扫码。

在微信客户端中扫描二维码。微信客户端可以通过扫一扫功能扫描二维码，获取二维码中的信息。

在后台中处理扫码请求。微信客户端扫描二维码后，会向后台发送请求，携带二维码中的信息。后台需要验证二维码信息是否正确，如果正确，则生成一个登录凭证，并将凭证返回给客户端。

在网页中验证凭证。客户端收到后台返回的凭证后，需要将凭证发送给后台验证，如果验证通过，则完成登录操作。

需要注意的是，微信扫码登录需要使用微信开放平台的API进行实现，具体实现方法可以参考微信开放平台的官方文档。
...
创建一个新的 Postman 集合，并在其中创建一个新的请求，填写请求的 URL 和参数等信息，并确保请求可以正常响应。

在 Postman 的测试标签页中编写一个 JavaScript 脚本来模拟多个用户同时访问该请求，并记录请求的响应时间和状态码等信息。

在 Postman 的 Runner 标签页中配置压力测试的参数，例如并发数、请求间隔时间、持续时间等，并选择要运行的测试集合和测试脚本。

启动测试，并观察测试结果，例如平均响应时间、错误率等指标，以评估系统的性能和稳定性。
