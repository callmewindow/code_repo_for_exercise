...
3. map(func)
   ...
   通过指定函数处理数组的每个元素，并返回处理后的数组，注意map是不会修改原来数组的，因此需要用变量接收
...
2. 位运算有范围
   ...
   对于位运算，包括应用，都只适用于32位的二进制数字范围，即上限为2147483647，下限为-2147483648
...
3. 判断奇偶
   ...
   1其实等于00...0001，因此任何数&1都相当于这个数的最后一位&1，因此奇数&1=1，偶数&1=0
   基于&1==1便可判断数字是奇数还是偶数
