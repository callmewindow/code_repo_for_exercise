...
1. Infinity
在除0时出现，有正负之分，正负之间为符号的差异，可通过Math.abs来进行比较
...
##### 优化为乘法
除法运算同样较为耗时，同时可能由于0值出现计算出错或者Infinity的情况
...
2. 1～n的和
n*(n+1)/2，判断大小可优化为n*(n+1) < 2*something

##### 优化为位运算

对于乘2，除2之类的操作可以替换为左移1和右移1
位运算的速度相比普通运算更加快速
...
3. 减少函数的调用和传值
例如console.log 会占用很多时间，提交时注意去除
以及使用min，max等函数，虽然好用，但是一定会增加运行时间
